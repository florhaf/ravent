...
public class Vote extends Model implements Serializable  {
	...
	public Vote(String accessToken, String userid, String eventid, String svote, boolean visibility, String message) {
	    ...
	    if (!visibility || (visibility && this.nb_vote == 1)) {
    		...
    		try {
        		client.publish(eventid + "/feed", FacebookType.class, Parameter.with("message", message), Parameter.with("link", "https://apps.facebook.com/gemsterapp/?eid=" + eventid),
    		...
    		if (!visibility) {
    			...
    			client.publish(userid + "/gemsterapp:drop_a_gem_on", FacebookType.class, Parameter.with("event", "https://apps.facebook.com/gemsterapp/?eid=" + eventid));
    		}
    	}
	}
